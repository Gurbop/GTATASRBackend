...
class Record(db.Model):
    ...
    decrypted_data = db.Column(db.String(512), nullable=False)  # New column for decrypted data
    ...
    def serialize(self):
        return {
            ...
            "decrypted_data": self.decrypted_data,  # Include decrypted data in the serialization
...
class RecordListResource(Resource):
    ...
    def post(self):
        try:
            ...
            decrypted_data = caesar_cipher_decrypt(reverse_string_by_swapping(final_encrypted_data), shift_value)

            new_record = Record(
                raw_data=raw_data, 
                encrypted_data=final_encrypted_data, 
                decrypted_data=decrypted_data,  # Store decrypted data
...
class RecordResource(Resource):
    ...
    def put(self, record_id):
        ...
        record.decrypted_data = decrypted_data  # Update decrypted data
...
class DecryptRecordResource(Resource):
    def get(self, encrypted_id):
        record = Record.query.filter_by(encrypted_id=encrypted_id).first()
        if not record:
            return jsonify({"message": "Record not found"}), 404
        ...
        return jsonify({
            ...
            "decrypted_data": record.decrypted_data  # Return decrypted data directly from the database
        })
...
api.add_resource(DecryptRecordResource, '/decrypt_data/<string:encrypted_id>')  # New endpoint for decryption
